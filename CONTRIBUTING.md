...
`.githooks/pre-push` refuses the push if any check fails, and
`.github/workflows/ci.yml` runs the same set, so a green push is a green pull request.

What those checks are, and how many, is listed in the README's *Working on it*
section. This paragraph used to keep its own copy of both. It said twelve, the gate
ran eighteen, and the list left out six of them, which is what a second copy is for.
