...
about six hundred of them to copy the shape from. We aim to acknowledge within a few
...
- **The HTTP listener binds loopback and has no TLS and no authentication.** That is
  the honest configuration for a plaintext listener, and it says so at startup. A
  routable bind is a deliberate operator act.
- **No gRPC, no authentication enforcement.** The `AuthProvider` contract and
  `Action::Ingest` exist and the server does not call them yet. That is written in
  the README's own list of what is unfinished.
