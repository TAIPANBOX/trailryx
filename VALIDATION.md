...
**Re-run 28 August 2026, same machine, same command, on a tree twenty days and a
RECORD_V2 migration later.** The figures reproduce: 65, 1,010, 7,328, 50,846 and
145,362 records per second, four of the five within 2% of the row above them. The
`sync_every = 1` row is 5.8% low and it is the noisy one by construction, since it
is the only policy that finishes a five-second window with a few hundred records
rather than a few hundred thousand; its run also took 5.9 s rather than 5.0, which
is the machine and not the code.

Recorded as a second dated measurement rather than as a replacement. A number here
is true about its moment, and two moments agreeing is worth more than one moment
overwritten. The claim the section rests on survives it: 145,362 over 65 is a
factor of 2,236.
